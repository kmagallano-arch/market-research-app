...
export default function PHMarketPage() {
  ...
  const filteredProducts = data?.trendingProducts?.filter((p:any) => {
    ...
    return p.platform?.toLowerCase().includes(PLATFORMS[platform]?.label.toLowerCase())
  }) || []
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div className="card" style={{ overflow:'hidden' }}>
              <div style={{ padding:'14px 20px', borderBottom:'1px solid #F0F1F5', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <span style={{ fontSize:12, color:'#9CA3AF' }}>{filteredProducts.length} products{platform!=='all'?` on ${PLATFORMS[platform]?.label}`:''}</span>
              </div>
              <table>
                <thead>
                ...
                <tbody>
                  {(platform==='all'?data.trendingProducts||[]:filteredProducts).map((p:any,i:number)=>(
                    <tr key={i}>
                      <td style={{ fontWeight:500, color:'#1A1D2E', maxWidth:240 }}>{p.title}</td>
                      <td>
                        <span style={{ fontSize:11, padding:'3px 9px', borderRadius:20, fontWeight:600, background:p.platform==='Lazada'?'rgba(15,20,109,0.1)':p.platform==='TikTok Shop'?'rgba(1,1,1,0.07)':'rgba(238,77,45,0.1)', color:p.platform==='Lazada'?'#0F146D':p.platform==='TikTok Shop'?'#333':'#EE4D2D' }}>
                          {p.platform}
                        </span>
                      </td>
