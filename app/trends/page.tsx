    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F2F3F7' }}>
      ...
      <main style={{ flex: 1, padding: '28px 32px', overflow: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
          {[
            { label: 'Rising Fast', value: SIGNALS.filter(s=>parseInt(s.change)>20).length, color: '#00C48C', bg: 'rgba(0,196,140,0.1)', sub: '>20% growth' },
            { label: 'US Dominant', value: SIGNALS.filter(s=>s.us>s.ph+10).length, color: '#2E6FFF', bg: 'rgba(46,111,255,0.1)', sub: 'US score >PH+10' },
            { label: 'PH Dominant', value: SIGNALS.filter(s=>s.ph>s.us).length, color: '#FF4D6A', bg: 'rgba(255,77,106,0.1)', sub: 'PH beating US' },
            { label: 'Both Markets', value: SIGNALS.filter(s=>s.us>70&&s.ph>70).length, color: '#FFB830', bg: 'rgba(255,184,48,0.1)', sub: 'Both score >70' },
          ].map((c,i)=>(
              ...
              {sorted.map((k,i)=>{
                ...
                return (
                  <tr key={i}>
                    ...
                    <td><span style={{ fontSize: 11, padding: '2px 9px', background: '#F3F4F6', color: '#6B7280', borderRadius: 20, fontWeight: 500 }}>{k.cat}</span></td>
