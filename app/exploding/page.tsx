...
const STAGE_CONFIG: Record<string,{color:string;bg:string;icon:string}> = {
  ...
  'early':    {color:'#2E6FFF',bg:'rgba(46,111,255,0.1)',icon:'📡'},
  'growing':  {color:'#FFB830',bg:'rgba(255,184,48,0.1)',icon:'📈'},
  'peaking':  {color:'#FF4D6A',bg:'rgba(255,77,106,0.1)',icon:'🔥'},
  'post-peak':{color:'#9CA3AF',bg:'rgba(156,163,175,0.1)',icon:'📉'},
}
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#F2F3F7'}}>
      ...
      <main style={{flex:1,padding:'28px 32px',overflow:'auto'}}>
        <div style={{marginBottom:24}}>
          <h1 style={{fontSize:24,fontWeight:700,color:'#1A1D2E',letterSpacing:'-0.4px'}}>💥 Exploding Topics</h1>
          <p style={{fontSize:14,color:'#6B7280',marginTop:3}}>Keyword velocity tracker — find trends before they peak</p>
        </div>
        ...
        <div style={{display:'flex',gap:10,marginBottom:20,alignItems:'center',flexWrap:'wrap'}}>
          ...
          {loading && <div style={{display:'flex',alignItems:'center',gap:6,fontSize:13,color:'#9CA3AF'}}><div style={{width:13,height:13,borderRadius:'50%',border:'2px solid #E8E9EF',borderTopColor:'#FF4D6A',animation:'spin 0.8s linear infinite'}}/>Analyzing velocity...</div>}
        </div>
            ...
            <tbody>
              {loading ? [...Array(12)].map((_,i)=>(
                ...
                return (
                  <tr key={i}>
                    <td>
                      <div style={{fontWeight:600,color:'#1A1D2E',marginBottom:2}}>{k.keyword}</div>
                      <div style={{fontSize:11,color:'#9CA3AF'}}>{k.category}</div>
                    </td>
                    ...
                    <td>
                      <div style={{display:'flex',alignItems:'center',gap:8}}>
                        <div style={{width:60,height:6,background:'#F0F1F5',borderRadius:3}}><div style={{width:`${k.velocityScore}%`,height:'100%',background:sc.color,borderRadius:3}}/></div>
                    ...
                    <td style={{fontSize:13,color:'#6B7280'}}>{k.peakEstimate}</td>
                    <td>
                      <div style={{display:'flex',gap:3,flexWrap:'wrap'}}>
                        {(k.markets||[]).slice(0,3).map((mkt:string,j:number)=>(
                          <span key={j} style={{fontSize:10,padding:'2px 6px',background:'#F3F4F6',color:'#6B7280',borderRadius:12}}>{mkt}</span>
                        ))}
                      </div>
                    </td>
