...
export default function RisingPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/rising?market=${market}`)
  ...
  const compColor=(c:string)=>({None:'#00C48C',Low:'#0EA5E9',Medium:'#FFB830'}[c]||'#94A3B8')
      ...
      <main style={{ flex:1, padding:'28px 32px', overflow:'auto' }}>
        <div style={{ marginBottom:24 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
            <h1 style={{ fontSize:22, fontWeight:700, color:'#0F172A', letterSpacing:'-0.4px' }}>🚀 Rising Products</h1>
          ...
          <p style={{ fontSize:13, color:'#64748B' }}>New-to-market products with strong search growth and conversion signals</p>
        </div>

        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:11, fontWeight:600, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:8 }}>Market</div>
          ...
          <div style={{ display:'flex', background:'#E2E8F0', borderRadius:10, padding:3 }}>
            {([['all','All'],['exploding','🔥'],['rising','📈'],['emerging','🌱']] as const).map(([val,label])=>(
              <button key={val} onClick={()=>setFilter(val)} style={{ padding:'5px 12px', borderRadius:8, border:'none', cursor:'pointer', fontSize:12, fontWeight:filter===val?700:400, background:filter===val?'white':'transparent', color:filter===val?'#0F172A':'#94A3B8', boxShadow:filter===val?'0 1px 4px rgba(0,0,0,0.1)':'none' }}>
                {label} {val==='all'&&`(${data.length})`}
              </button>
            ))}
          </div>
          {loading && <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'#94A3B8' }}><div style={{ width:12, height:12, borderRadius:'50%', border:'2px solid #E2E8F0', borderTopColor:m.color, animation:'spin 0.8s linear infinite' }}/>Scanning...</div>}
        </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
            {[
              ...
              <div key={i} className="card" style={{ padding:'16px 18px' }}>
                <div style={{ fontSize:10, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:6 }}>{c.label}</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(340px,1fr))', gap:14 }}>
          {loading ? [...Array(6)].map((_,i)=><div key={i} className="card skeleton" style={{height:280}}/>) :
              ...
              <div key={i} className="card" style={{ padding:'18px 20px' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                  ...
                  <div style={{ textAlign:'right' }}>
                    ...
                    <div style={{ fontSize:9, color:'#94A3B8', textTransform:'uppercase' }}>Momentum</div>
                  </div>
                </div>
                <div style={{ fontSize:15, fontWeight:700, color:'#0F172A', marginBottom:3 }}>{p.name}</div>
                <div style={{ fontSize:11, color:'#94A3B8', marginBottom:10 }}>{p.category} · {p.launchAge} old</div>
                <div style={{ fontSize:12, color:'#64748B', lineHeight:1.5, marginBottom:12, padding:'8px 12px', background:'#F1F5F9', borderRadius:8, borderLeft:`3px solid ${tc.color}` }}>{p.whyNow}</div>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6, marginBottom:12 }}>
                  {[{v:p.searchGrowth,l:'Search',c:'#00C48C'},{v:p.conversionRate,l:'Conv.',c:'#0EA5E9'},{v:p.avgPrice,l:'Price',c:'#FFB830'}].map((s,j)=>(
                ...
                <div style={{ marginBottom:10 }}>
                  {(p.earlySignals||[]).slice(0,3).map((s:string,j:number)=>(
                    <div key={j} style={{ display:'flex', gap:6, marginBottom:4 }}>
                      ...
                      <span style={{ fontSize:11, color:'#64748B', lineHeight:1.4 }}>{s}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display:'flex', justifyContent:'space-between', paddingTop:10, borderTop:'1px solid #E2E8F0' }}>
                  ...
                  <span style={{ fontSize:11, color:'#94A3B8', fontFamily:'DM Mono' }}>{p.monthlySales}</span>
                </div>
