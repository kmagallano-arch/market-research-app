    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F1F5F9' }}>
      ...
      <main style={{ flex: 1, padding: '28px 32px', overflow: 'auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#0F172A', letterSpacing: '-0.4px' }}>🔍 Keyword Search</h1>
          <p style={{ fontSize: 14, color: '#64748B', marginTop: 3 }}>Live search volume, competition, CPC and related keywords</p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 14, alignItems: 'center' }}>
          <input
            ...
            style={{ flex: 1, maxWidth: 440, padding: '10px 16px', borderRadius: 10, border: '1.5px solid #E2E8F0', fontSize: 14, outline: 'none', fontFamily: 'DM Sans, sans-serif' }}
          />
          <button
            onClick={() => search()}
            disabled={state === 'loading'}
            style={{ padding: '10px 24px', background: state === 'loading' ? '#94A3B8' : m.color, color: 'white', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: state === 'loading' ? 'not-allowed' : 'pointer', transition: 'background 0.2s' }}
          >
        ...
        {state === 'loading' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '24px 20px', color: '#94A3B8', fontSize: 14 }}>
            ...
            Searching for <b style={{ color: '#0F172A', marginLeft: 4 }}>{searched}</b>...
          </div>
        )}
        ...
        {state === 'done' && data && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {data.main.seasonality && <span style={{ fontSize: 12, color: '#94A3B8' }}>📅 {data.main.seasonality}</span>}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12 }}>
              {[
                ...
                <div key={i} className="card" style={{ padding: '16px 18px' }}>
                  <div style={{ fontSize: 11, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>{s.label}</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div className="card" style={{ padding: '20px 22px' }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#0F172A', marginBottom: 16 }}>📈 12-Month Search Trend</div>
                ...
                ) : <div style={{ color: '#94A3B8', fontSize: 13 }}>No trend data available</div>}
                {data.main.opportunity && (
                  <div style={{ marginTop: 14, padding: '10px 14px', background: `${m.color}0D`, borderRadius: 8, fontSize: 13, color: '#64748B', lineHeight: 1.5, borderLeft: `3px solid ${m.color}` }}>
                    💡 {data.main.opportunity}
                  </div>
                )}
              </div>
              ...
              <div className="card" style={{ padding: '20px 22px' }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#0F172A', marginBottom: 14 }}>📌 Insights</div>
                {(data.insights || []).map((ins: string, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: 10, padding: '10px 0', borderBottom: '1px solid #E2E8F0', alignItems: 'flex-start' }}>
                    <span style={{ color: m.color, flexShrink: 0, fontWeight: 700, fontSize: 16 }}>›</span>
                    <span style={{ fontSize: 13, color: '#64748B', lineHeight: 1.5 }}>{ins}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #E2E8F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ fontSize: 15, fontWeight: 700, color: '#0F172A' }}>Related Keywords <span style={{ fontSize: 13, color: '#94A3B8', fontWeight: 400 }}>({sortedRelated.length})</span></div>
                <div style={{ display: 'flex', gap: 6 }}>
                  {(['volume', 'difficulty', 'cpc'] as const).map(s => (
                    <button key={s} onClick={() => setSortBy(s)}
                      style={{ padding: '4px 12px', borderRadius: 20, border: `1.5px solid ${sortBy === s ? m.color : '#E2E8F0'}`, background: sortBy === s ? `${m.color}12` : 'white', color: sortBy === s ? m.color : '#94A3B8', fontSize: 12, cursor: 'pointer', fontWeight: sortBy === s ? 700 : 400, textTransform: 'capitalize' }}>
              ...
              <table>
                ...
                <tbody>
                  {sortedRelated.map((k: any, i: number) => {
                    ...
                    return (
                      <tr key={i}>
                        <td style={{ fontWeight: 500, color: '#0F172A', maxWidth: 220 }}>{k.keyword}</td>
                        <td style={{ fontFamily: 'DM Mono', fontSize: 13, color: '#0F172A', fontWeight: 600 }}>{k.searchVolume || '—'}</td>
                        <td>
