...
const TREND_CONFIG: Record<string,{color:string;bg:string;icon:string}> = {
  ...
  trending:  {color:'#2E6FFF',bg:'rgba(46,111,255,0.1)',icon:'📈'},
  fading:    {color:'#9CA3AF',bg:'rgba(156,163,175,0.1)',icon:'📉'},
}
      ...
      <main style={{flex:1,padding:'28px 32px',overflow:'auto'}}>
        <div style={{marginBottom:24}}>
          <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4}}>
            <h1 style={{fontSize:24,fontWeight:700,color:'#1A1D2E',letterSpacing:'-0.4px'}}>🎵 TikTok Viral Tracker</h1>
          ...
          <p style={{fontSize:14,color:'#6B7280',marginTop:3}}>Products going viral on TikTok Shop — spot trends before they hit Amazon</p>
        </div>
        ...
        <div style={{display:'flex',gap:10,marginBottom:20,alignItems:'center',flexWrap:'wrap'}}>
          ...
          <div style={{display:'flex',background:'#EBEBEF',borderRadius:10,padding:3}}>
            {['all','exploding','viral','trending','fading'].map(val=>(
              <button key={val} onClick={()=>setFilter(val)} style={{padding:'5px 13px',borderRadius:8,border:'none',cursor:'pointer',fontSize:12,fontWeight:filter===val?700:400,background:filter===val?'white':'transparent',color:filter===val?'#1A1D2E':'#9CA3AF',boxShadow:filter===val?'0 1px 4px rgba(0,0,0,0.1)':'none',textTransform:'capitalize'}}>
                {val==='all'?`All (${data.length})`:val}
              </button>
            ))}
          </div>
          {loading && <div style={{display:'flex',alignItems:'center',gap:6,fontSize:13,color:'#9CA3AF'}}><div style={{width:13,height:13,borderRadius:'50%',border:'2px solid #E8E9EF',borderTopColor:'#FE2C55',animation:'spin 0.8s linear infinite'}}/>Scanning TikTok...</div>}
        </div>
              ...
              <div key={i} className="card" style={{padding:'18px 20px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                  ...
                  <div style={{textAlign:'right'}}>
                    ...
                    <div style={{fontSize:9,color:'#9CA3AF',textTransform:'uppercase'}}>Score</div>
                  </div>
                </div>
                <div style={{fontSize:15,fontWeight:700,color:'#1A1D2E',marginBottom:2}}>{p.productName}</div>
                <div style={{fontSize:13,color:'#FE2C55',fontWeight:600,marginBottom:8}}>{p.hashtag}</div>
                <div style={{fontSize:12,color:'#6B7280',lineHeight:1.5,marginBottom:12,padding:'8px 12px',background:'#F8F9FB',borderRadius:8,borderLeft:'3px solid #FE2C55'}}>{p.whyViral}</div>
                ...
                <div style={{display:'flex',justifyContent:'space-between',paddingTop:10,borderTop:'1px solid #F0F1F5',fontSize:12}}>
                  <span style={{color:'#6B7280'}}>{p.creatorCount} creators · {p.ageGroup}</span>
