...
const MARKET_CODE = 'NO'
const MARKET_NAME = 'Norway'
const MARKET_FLAG = '🇳🇴'
const MARKET_COLOR = '#EC4899'
const MARKET_PLATFORMS = 'Amazon NO, Finn.no, Elkjop'

export default function NorwayMarketPage() {
  const [data, setData] = useState<any>(null)
  const [loading, setLoading] = useState(true)
  const [platform, setPlatform] = useState<PlatformKey>('all')

  useEffect(() => {
    fetch('/api/no-market').then(r=>r.json()).then(j=>{setData(j.data); setLoading(false)})
  }, [])
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#F2F3F7'}}>
      ...
      <main style={{flex:1,padding:'28px 32px',overflow:'auto'}}>
        <div style={{marginBottom:24}}>
          <h1 style={{fontSize:24,fontWeight:700,color:'#1A1D2E',letterSpacing:'-0.4px'}}>{MARKET_FLAG} {MARKET_NAME} Market</h1>
          <p style={{fontSize:14,color:'#6B7280',marginTop:3}}>{MARKET_PLATFORMS} — trending products, categories, market intelligence</p>
        </div>
        ...
        {loading ? (
          <div style={{display:'flex',alignItems:'center',gap:10,padding:'32px 20px',color:'#9CA3AF',fontSize:14}}>
            <div style={{width:16,height:16,borderRadius:'50%',border:'2px solid #E8E9EF',borderTopColor:MARKET_COLOR,animation:'spin 0.8s linear infinite'}}/>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))',gap:12}}>
              {(data.topCategories||[]).map((cat:any,i:number)=>(
                <div key={i} className="card" style={{padding:'18px 20px'}}>
                  <div style={{fontSize:14,color:'#1A1D2E',fontWeight:600,marginBottom:8}}>{cat.name}</div>
                  <div style={{fontSize:26,fontWeight:800,color:'#00C48C',fontFamily:'DM Mono'}}>{cat.growth}</div>
                  <div style={{fontSize:12,color:'#9CA3AF',marginTop:4}}>avg {cat.avgPrice}</div>
                </div>
              ))}
            </div>
            ...
            <div className="card" style={{overflow:'hidden'}}>
              <div style={{padding:'16px 20px',borderBottom:'1px solid #F0F1F5',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <span style={{fontSize:13,color:'#9CA3AF'}}>{filteredProducts.length} products{platform!=='all'?` · ${PLATFORMS[platform]?.label}`:''}</span>
              </div>
              <table>
                <thead><tr><th>Product</th><th>Platform</th><th>Price</th><th>Sold/mo</th><th>Rating</th><th>Trend</th><th>Keywords</th></tr></thead>
                <tbody>
                  {filteredProducts.length===0 ? (
                    <tr><td colSpan={7} style={{textAlign:'center',padding:'32px',color:'#9CA3AF',fontSize:14}}>No products for this platform. Try "All Platforms".</td></tr>
                  ) : filteredProducts.map((prod:any,i:number)=>{
                    const pb = platformBadge(prod.platform)
                    return (
                      <tr key={i}>
                        <td style={{fontWeight:500,color:'#1A1D2E',maxWidth:240}}>{prod.title}</td>
                        ...
                        <td style={{color:'#6B7280',fontFamily:'DM Mono',fontSize:13}}>{prod.soldLastMonth}</td>
