  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F2F3F7' }}>
      ...
      <main style={{ flex: 1, padding: '28px 32px', overflow: 'auto' }}>
        ...
        {/* Filter tabs */}
        <div style={{ display: 'flex', gap: 2, marginBottom: 20, background: '#EBEBEF', borderRadius: 11, padding: 3, width: 'fit-content' }}>
          {(['ALL','US','PH','BOTH'] as const).map(val => (
            <button key={val} onClick={()=>setFilter(val)} style={{ padding: '7px 20px', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: filter===val?700:400, background: filter===val?'white':'transparent', color: filter===val?'#1A1D2E':'#9CA3AF', transition: 'all 0.15s', boxShadow: filter===val?'0 1px 4px rgba(0,0,0,0.1)':'none' }}>
            ...
            <tbody>
              {loading ? [...Array(8)].map((_,i)=>(
                ...
                <tr key={i}>
                  ...
                  <td>
                    <span className={`badge badge-${r.targetMarket==='US'?'us':r.targetMarket==='PH'?'ph':'both'}`}>
