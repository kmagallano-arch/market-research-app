        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{
              ...
              border: `1px solid ${filter === 'ALL' ? '#2E6FFF' : '#EAECF0'}`,
              background: filter === 'ALL' ? '#EFF8FF' : 'white',
              color: filter === 'ALL' ? '#175CD3' : '#475467',
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${active ? color : '#EAECF0'}`,
                  background: active ? `${color}12` : 'white',
                  color: active ? color : '#475467',
              ...
              >
                ...
                <span>{code}</span>
                {count > 0 && (
                  <span style={{ fontSize: 11, background: active ? `${color}20` : '#F2F4F7', color: active ? color : '#98A2B3', padding: '1px 6px', borderRadius: 99, fontWeight: 600 }}>
                    {count}
                  </span>
                )}
              </button>
          ...
          <button
            ...
            style={{ marginLeft: 'auto', padding: '7px 14px', borderRadius: 8, border: '1px solid #EAECF0', background: 'white', color: '#475467', fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'Geist, sans-serif' }}
          >
          ...
          <table>
            ...
            <tbody>
              {loading ? [...Array(10)].map((_, i) => (
                ...
                <tr><td colSpan={8} style={{ textAlign: 'center', color: '#98A2B3', padding: '32px' }}>No picks for this market yet — click Refresh</td></tr>
              ) : filtered.map((r: any, i: number) => (
                <tr key={i}>
                  <td>
                    <div style={{ width: 40, height: 40, borderRadius: 10, background: r.score >= 80 ? 'linear-gradient(135deg,#12B76A,#38EF7D)' : r.score >= 60 ? 'linear-gradient(135deg,#2E6FFF,#818CF8)' : 'linear-gradient(135deg,#F79009,#FF8C42)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: 'white', fontFamily: 'Geist Mono, monospace' }}>
                      {r.score}
                    </div>
                  </td>
