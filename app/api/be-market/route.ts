...
export async function GET(req: NextRequest) {
  ...
  const cacheKey = `be-market:${category}`

  try {
    const { data, cached, ageMinutes } = await withCache(cacheKey, async () => {
      const result = await askOpenAIJSON<any>(`
You are a Belgium e-commerce market research expert.
Market context: Belgian e-commerce. Bilingual market (French/Dutch). Quality and sustainability focused.
Category: "${category}"
Platforms: Bol.com, Amazon, Fnac, Coolblue
...
Return JSON: {
  ...
  "trendingProducts": [{
    ...
    "brand": "Brand name",
    "platform": "one of: Bol.com, Amazon, Fnac, Coolblue",
    "price": "€XXX",
    "originalPrice": "€XXX",
    "soldLastMonth": "X,XXX",
...
Mix platforms naturally. Be specific with Belgium market context. Include local brands.`)
      return result
    }, 60)
