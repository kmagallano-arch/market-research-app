...
export async function GET(req: NextRequest) {
  ...
  const cacheKey = `nl-market:${category}`

  try {
    const { data, cached, ageMinutes } = await withCache(cacheKey, async () => {
      const result = await askOpenAIJSON<any>(`
You are a Netherlands e-commerce market research expert.
Market context: Dutch e-commerce. Sustainability-conscious. Bol.com dominant. Cycling/outdoor culture.
Category: "${category}"
Platforms: Bol.com, Amazon, Coolblue
...
Return JSON: {
  ...
  "trendingProducts": [{
    ...
    "brand": "Brand name",
    "platform": "one of: Bol.com, Amazon, Coolblue",
    "price": "€XXX",
    "originalPrice": "€XXX",
    "soldLastMonth": "X,XXX",
...
Mix platforms naturally. Be specific with Netherlands market context. Include local brands.`)
      return result
    }, 60)
