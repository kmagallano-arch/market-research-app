...
export async function GET(req: NextRequest) {
  ...
  const cacheKey = `fr-market:${category}`

  try {
    const { data, cached, ageMinutes } = await withCache(cacheKey, async () => {
      const result = await askOpenAIJSON<any>(`
You are a France e-commerce market research expert.
Market context: French e-commerce. Style and quality focused. Strong beauty and home décor market.
Category: "${category}"
Platforms: Amazon, Cdiscount, Fnac, Darty
...
Return JSON: {
  ...
  "trendingProducts": [{
    ...
    "brand": "Brand name",
    "platform": "one of: Amazon, Cdiscount, Fnac, Darty",
    "price": "€XXX",
    "originalPrice": "€XXX",
    "soldLastMonth": "X,XXX",
...
Mix platforms naturally. Be specific with France market context. Include local brands.`)
      return result
    }, 15)
