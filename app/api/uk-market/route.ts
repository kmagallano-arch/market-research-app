...
export async function GET(req: NextRequest) {
  ...
  const cacheKey = `uk-market:${category}`

  try {
    const { data, cached, ageMinutes } = await withCache(cacheKey, async () => {
      const result = await askOpenAIJSON<any>(`
You are a United Kingdom e-commerce market research expert.
Market context: UK e-commerce. Value-conscious but quality-focused. Brexit import considerations.
Category: "${category}"
Platforms: Amazon, eBay, TikTok Shop, Argos
...
Return JSON: {
  ...
  "trendingProducts": [{
    ...
    "brand": "Brand name",
    "platform": "one of: Amazon, eBay, TikTok Shop, Argos",
    "price": "£XXX",
    "originalPrice": "£XXX",
    "soldLastMonth": "X,XXX",
...
Generate 8 top categories, 30 trending products in price range £8-£150.
Mix platforms naturally. Be specific with United Kingdom market context. Include local brands.`)
      return result
    }, 15)
